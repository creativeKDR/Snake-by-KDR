...
intro = pygame.image.load("images/welcome.jpg")
intro = pygame.transform.scale(intro, (screen_width, screen_height)).convert_alpha()

# Title and display
pygame.display.set_caption("Snake by KDR")
...
def welcome():
    ...
    while not exit_game:
        ...
        #text_screen("Press Space to Play",red,260,290)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                exit_game = True
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    pygame.mixer.music.load('music/gameBG.mp3')
                    pygame.mixer.music.play()
                    gameloop()
        pygame.display.update()
        clock.tick(30)
...
def gameloop():
    ...
    while not exit_game:
        if game_over:
            ...
            text_screen("Hiscore: "+str(hiscore),blue,660,5)
